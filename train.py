...
from core.patches.aim_patches import S3ArtifactStoragePatcher, AimLoggerWithContext
from core.patches.model_summary_patch import ModelSummaryWithGradCallback
from core.callbacks import SaveValidationResults, PlotValidationResults, BarPlotMetricAim, PlotConfusionMetricAim
from core.datasets import ImageListsWithLabelIndex
from core.models import SupervisedModel, get_model_base_transforms, check_model_name
...
def argparse_init(parser=None):
    ...
    # BASE TRANSFORMS #
    #dataset.add_argument('--img-norm', nargs=2, metavar=('MEAN', 'STD'),
...
def argparse_runtime_args(args):
    ...
    if args.artifacts_location and os.path.isfile(args.artifacts_location):
...
def main(args):
    ...
    logger_kwargs = dict(experiment=args.experiment, run_name=args.run,
                         context_postfixes=dict(
                             ...
                             rebalanced={'yes': '_rebalanced', 'no': '_unbalanced'}))  # loss
    ...
    if args.artifacts_location or ( 'AIM_ARTIFACTS_URI' in os.environ and os.environ['AIM_ARTIFACTS_URI'] ):
        if os.path.isfile(args.classlist):
            [logger.experiment.log_artifact(args.classlist, name=os.path.basename(args.classlist)) for logger in loggers]
        if os.path.isfile(args.vallist):
            [logger.experiment.log_artifact(args.trainlist, name=os.path.basename(args.vallist)) for logger in loggers]
        if os.path.isfile(args.trainlist):
            [logger.experiment.log_artifact(args.trainlist, name=os.path.basename(args.trainlist)) for logger in loggers]
    ...
    if args.artifacts_location or ( 'AIM_ARTIFACTS_URI' in os.environ and os.environ['AIM_ARTIFACTS_URI'] ):
        [logger.experiment.log_artifact(trainer.checkpoint_callback.best_model_path, name='best.ckpt') for logger in trainer.loggers]

    # Copying Logs + cleanup logs (remove "steps", re-order columns)

    # ONNX Export
    # TODO as a callback
    if args.onnx:
        outfile = os.path.join(args.outdir, 'best.onnx')
        os.makedirs(os.path.dirname(outfile), exist_ok=True)
        datamodule.batch_size = 10
        sample_dataloader = datamodule.train_dataloader()
        for batch in sample_dataloader:
            model.to_onnx(outfile, batch[0], export_params=True)
            ## todo ADL. ARGS  ##
